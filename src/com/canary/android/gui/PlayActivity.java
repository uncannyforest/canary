...
public class PlayActivity extends AppCompatActivity {
    ...
    private void saveAudio() {
        final AlertDialog dialog = alert("Save", "Please wait");
        Thread thread = new Thread() {
            @Override
            public void run() {
                try {
                    File file = getExportFile();
                    WavWriter.save(file, 44100, 16, playManager.getSource());
                    dialog.setMessage("WAV file successfully saved to " + file.toString());
    ...
    private void scaleImage(BitmapDrawable drawable) {
        ...
        Log.d("layoutParams", "" + ((AudioManager)(this.getSystemService(Context.AUDIO_SERVICE))).getProperty(AudioManager.PROPERTY_OUTPUT_SAMPLE_RATE));
    ...
    private File getCacheFile() {
        return null;//new File(context.getCacheDir(), "song.wav");
    }
